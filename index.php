...
?>
	  ...
	  <input class="hidden" id="<?echo $i["id"];?>" type="hidden" name="hidden" value="<?echo $i["id"];?>">
...
// $invoice_number_data = $_POST["invoice_number"];
//
// if ($stmt = $sql->prepare("SELECT `invoice_number` FROM `invoices` WHERE `invoice_number` = $invoice_number_data LIMIT 1") AND $stmt->execute(array()) AND $data = $stmt->fetchAll()) {
// 					foreach ($data as $i) {
//
// 					if (strlen($i["invoice_number"]) > 0)
// 						{
// 							$duplicate = "duplicate";
// 						}
//
...
?>
    ...
    $("#collection_form").on("submit", function(e){
            ...
            success:function(response){
							...
							// < if ($stmt = $sql->prepare($last_id) AND $stmt->execute(array("1")) AND $data = $stmt->fetch()) { ?>
            ...
            error:function (xhr, ajaxOptions, thrownError){
                alert(thrownError);
					...
					$("body").on("click", ".del_button", function(e) {
			          e.preventDefault();
			          var DbNumberID = $(this).attr('id'); //and get number from array
			          var myData = "recordToDelete=" + DbNumberID; //build a post data structure
			        //  $('#row_'+DbNumberID).addClass( "sel" ); //change background of this element by adding class
