...
<html>
		...
		<br>
	  	Vendor:<br>
			<select name="vendor" id="vendor">
				<?
				if ($stmt = $mysqli->prepare($vendor_data) AND $stmt->execute(array()) AND $data = $stmt->fetchAll()) {
					foreach ($data as $i) {
						?>
  ...
  <tr id="append_table">
	...
	// 	if ($stmt = $mysqli->prepare($invoice_data) AND $stmt->execute(array()) AND $data = $stmt->fetchAll()) {
	// 		foreach ($data as $row) {
	// 	  echo '<li id="item_'.$row["invoice_date"].'">';
	// 	  echo '<div class="del_wrapper"><a href="#" class="del_button" id="del-'.$row["invoice_date"].'">';
	// 	  echo '</a></div>';
	// 	  echo $row["invoice_number"].'</li>';
	// 	}
	// }
?>
		...
		$(document).ready(function() {
			...
			if ($stmt = $mysqli->prepare($invoice_data) AND $stmt->execute(array()) AND $data = $stmt->fetchAll()) {
				foreach ($data as $i) {
						...
						?>
						$("#append_table").append("<? echo "<td>" . $i["invoice_date"] . "</td>"; ?>");
						$("#append_table").append("<? echo "<td>" . $i["invoice_number"] . "</td>"; ?>");
						$("#append_table").append("<? echo "<td>" . $i["invoice_vendor"] . "</td>"; ?>");
						$("#append_table").append("<? echo "<td>" . $i["subtotal"] . "</td>"; ?>");
						$("#append_table").append("<? echo "<td>" . $i["total"] . "</td>"; ?>");
						<?
				}
			}
			?>
